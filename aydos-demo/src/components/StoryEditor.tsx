...
import { fetchStoryText, parseStoryTxt } from "../utils/storyTextLoader";
import { saveQuizForStory } from "../utils/quizStorage";
import { parseTagsString } from "../utils/storiesTxt";
...
// Converts a title into a lowercase, underscore-separated folder-safe name
// e.g. "The Lotus Eaters" -> "the_lotus_eaters" -- matches the /public/stories/ convention
...
export const StoryEditor: React.FC<StoryEditorProps> = ({
  ...
  const [estimatedMinutes, setEstimatedMinutes] = useState<number>(
    initialStory?.estimatedMinutes || 3
  );
  ...
  const handleAddTag = (rawTag?: string) => {
    ...
    // Support bracketed tags e.g. "[mythology] [homer]" or comma/space separated
  ...
  const [seriesAuthor, setSeriesAuthor] = useState(
    initialStory?.seriesAuthor || ""
  );
  const [chapterNumber, setChapterNumber] = useState(
    initialStory?.chapterNumber?.toString() || "Chapter I"
  );
  ...
  const [seriesOrder, setSeriesOrder] = useState<number>(
    initialStory?.seriesOrder ?? 1
  );

  // GitHub Asset Folder naming -- drives the automated asset resolution & "Use Folder Path"
  // Matching https://github.com/jamesfreda53-ops/aydos-demo/ (aydos-demo/public/stories/...)
  const [seriesFolder, setSeriesFolder] = useState(
    () => initialStory?.seriesFolder || (initialStory?.seriesTitle ? slugify(initialStory.seriesTitle) : "")
  );
  const [storyFolderName, setStoryFolderName] = useState(
    () => initialStory?.storyFolder || (initialStory?.id && !initialStory.id.startsWith("custom-") ? initialStory.id : slugify(initialStory?.title || "new_story"))
  );
  const [chapterFolderName, setChapterFolderName] = useState(
    () => initialStory?.chapterFolder || (initialStory?.chapterTitle ? slugify(initialStory.chapterTitle) : "")
  );

  // GitHub Auto-Fetch State
  const [isFetchingFromGithub, setIsFetchingFromGithub] = useState(false);
  ...
  // Resolves to GitHub folder path for standalone stories or series chapters
  const currentSeriesFolder = seriesFolder.trim() || (seriesTitle ? slugify(seriesTitle) : "series");
  const currentChapterFolder = chapterFolderName.trim() || (chapterTitle ? slugify(chapterTitle) : "chapter");
  const currentStoryFolder = storyFolderName.trim() || slugify(title) || "story";

  const basePath = isSeries
    ? `${GITHUB_RAW_BASE_URL}/${currentSeriesFolder}/${currentChapterFolder}`
    : `${GITHUB_RAW_BASE_URL}/${currentStoryFolder}`;
  ...
  const buildStoryObject = (): Story => {
    ...
    const titleSlideObj: TitleSlide = {
      ...
      subtitle,
      author,
      illustrationUrl: coverImage || (isSeries ? seriesTitleImage(currentSeriesFolder) : `${GITHUB_RAW_BASE_URL}/${currentStoryFolder}/images/title.webp`),
      illustrationCaption: `${title} - Title Artwork`,
      introParagraph: titleIntroParagraph,
    };

    const parsedOrderNum = typeof seriesOrder === "number" ? seriesOrder : parseInt(String(seriesOrder || chapterNumber).replace(/[^0-9]/g, ""), 10);
    const validOrder = !isNaN(parsedOrderNum) && parsedOrderNum > 0 ? parsedOrderNum : 1;

    return {
      id: initialStory?.id || (isSeries ? `${currentSeriesFolder}-${currentChapterFolder}` : currentStoryFolder) || `custom-${Date.now()}`,
      title,
      subtitle,
      author,
      coverImage: coverImage || titleSlideObj.illustrationUrl,
      titleSlide: titleSlideObj,
      readingLevel,
      levelBadge,
      estimatedMinutes: estimatedMinutes > 0 ? estimatedMinutes : Math.max(2, Math.round(slides.length * 0.7)),
      genre,
      themeColor: initialStory?.themeColor || "from-amber-600 to-orange-700",
      accentColor: initialStory?.accentColor || "#f97316",
      summary: summary.trim() || titleIntroParagraph.trim(),
      tags: tags.length > 0 ? tags : undefined,
      slides: slides.map((s, idx) => ({ ...s, slideNumber: idx + 1 })),
      quizQuestions,

      // Series metadata
      seriesId: isSeries ? (currentSeriesFolder || slugify(seriesTitle || "series")) : undefined,
      seriesTitle: isSeries ? (seriesTitle.trim() || "Series") : undefined,
      seriesAuthor: isSeries ? (seriesAuthor.trim() || undefined) : undefined,
      seriesFolder: isSeries ? currentSeriesFolder : undefined,
      storyFolder: !isSeries ? currentStoryFolder : undefined,
      chapterNumber: isSeries ? (chapterNumber.trim() || `Chapter ${validOrder}`) : undefined,
      chapterTitle: isSeries ? (chapterTitle.trim() || title.replace(/^Chapter\s+[IVXLCDM\d]+[:\s-]+/i, "").trim()) : undefined,
      chapterFolder: isSeries ? currentChapterFolder : undefined,
      seriesDescription: isSeries ? (seriesDescription.trim() || undefined) : undefined,
      seriesCoverImage: isSeries ? (initialStory?.seriesCoverImage || seriesTitleImage(currentSeriesFolder)) : undefined,
      seriesOrder: isSeries ? validOrder : undefined,
      userEdited: true,
      isCustom: true,
    };
  };
  ...
  const handleSave = () => {
    ...
    onSaveStory(story);
    saveQuizForStory(story, quizQuestions).catch((err) => {
      console.warn("Failed to save quiz.json:", err);
    });
  };
  ...
  // Auto-fill path buttons -- set the field to the expected GitHub path
  const applyFolderPathToCover = () => {
    if (isSeries) {
      setCoverImage(chapterTitleImage(currentSeriesFolder, currentChapterFolder));
    } else {
      setCoverImage(storyImage(currentStoryFolder, "title.webp"));
    }
  };

  const applyFolderPathToSlideImage = () => {
    const slideNum = activeSlideIndex + 1;
    const url = isSeries
      ? chapterImage(currentSeriesFolder, currentChapterFolder, `${slideNum}.webp`)
      : storyImage(currentStoryFolder, `${slideNum}.webp`);
    updateCurrentSlide({ illustrationUrl: url });
  };

  const applyFolderPathToSlideAudio = () => {
    const slideNum = activeSlideIndex + 1;
    const url = isSeries
      ? chapterAudio(currentSeriesFolder, currentChapterFolder, slideNum)
      : storyAudio(currentStoryFolder, slideNum);
    updateCurrentSlide({ audioUrl: url });
  };

  // Apply GitHub folder paths to all slides at once
  const handleApplyFolderPathsToAllSlides = () => {
    const updatedSlides = slides.map((s, idx) => {
      const slideNum = idx + 1;
      const imgUrl = isSeries
        ? chapterImage(currentSeriesFolder, currentChapterFolder, `${slideNum}.webp`)
        : storyImage(currentStoryFolder, `${slideNum}.webp`);
      const audUrl = isSeries
        ? chapterAudio(currentSeriesFolder, currentChapterFolder, slideNum)
        : storyAudio(currentStoryFolder, slideNum);
    ...
    applyFolderPathToCover();
    setFetchStatusMessage({
      text: `Applied GitHub paths (${isSeries ? `${currentSeriesFolder}/${currentChapterFolder}` : currentStoryFolder}) to all ${slides.length} slides!`,
      type: "success",
    });
  };

  // Auto-fetch story text and hydrate slides from GitHub repository
  const handleFetchFromGithubFolder = async () => {
    setIsFetchingFromGithub(true);
    setFetchStatusMessage({
      text: `Connecting to GitHub repository to load story assets...`,
      type: "info",
    });

    try {
      const sFolder = isSeries ? currentSeriesFolder : currentStoryFolder;
      const cFolder = isSeries ? currentChapterFolder : undefined;
      const sId = initialStory?.id || sFolder;

      const rawText = await fetchStoryText(sFolder, cFolder, sId, [
        title,
        chapterTitle,
        seriesTitle,
      ]);
      ...
      if (rawText) {
        ...
        if (parsed.titleIntro) {
          setTitleIntroParagraph(parsed.titleIntro);
          setSummary(parsed.titleIntro);
        } else if (parsed.summary) {
          setTitleIntroParagraph(parsed.summary);
          setSummary(parsed.summary);
        }

        if (parsed.metadata.title && (!title || title === "Untitled Story" || title.startsWith("New "))) {
          setTitle(parsed.metadata.title);
        }
        if (parsed.metadata.author && (!author || author === "Author")) {
          setAuthor(parsed.metadata.author);
        }
        if (parsed.metadata.seriesauthor && !seriesAuthor) {
          setSeriesAuthor(parsed.metadata.seriesauthor);
        }
        ...
        // Build or hydrate slides from parsed text
        const slideCount = Math.max(slides.length, parsed.slides.length, 7);
        const newSlides: Slide[] = [];

        for (let i = 0; i < slideCount; i++) {
          const slideNum = i + 1;
          const parsedSlide = parsed.slides.find((s) => s.slideNumber === slideNum) || parsed.slides[i];
          const existingSlide = slides[i];
          ...
          newSlides.push({
            id: existingSlide?.id || `slide-${Date.now()}-${slideNum}`,
            slideNumber: slideNum,
            paragraph: parsedSlide?.paragraph || existingSlide?.paragraph || `Paragraph for Slide ${slideNum}`,
            illustrationCaption: parsedSlide?.caption || existingSlide?.illustrationCaption || `Slide ${slideNum}`,
            illustrationUrl: imgUrl,
            audioUrl: audUrl,
          });
        }
        ...
        setFetchStatusMessage({
          text: `Loaded ${parsed.slides.length} slides with synchronized text, GitHub images, and audio narration!`,
          type: "success",
        });
      } else {
        // Fallback: apply GitHub paths to all slides
        handleApplyFolderPathsToAllSlides();
        setFetchStatusMessage({
          text: `Applied GitHub image/audio paths for "${isSeries ? `${currentSeriesFolder}/${currentChapterFolder}` : currentStoryFolder}". (No .txt found on GitHub yet; you can type the text below).`,
          type: "info",
        });
      }
    } catch (err: any) {
      ...
      handleApplyFolderPathsToAllSlides();
      setFetchStatusMessage({
        text: `Applied standard GitHub paths. Could not load .txt file: ${err?.message || "Network issue"}`,
        type: "error",
      });
    } finally {
      setIsFetchingFromGithub(false);
    }
  };
  ...
  const handleLoadSample = () => {
    if (
      ...
      setTags(["clockwork", "dragonfly", "steampunk", "adventure", "science", "invention", "nature"]);
      setTitleIntroParagraph(
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div className="lg:col-span-7 space-y-4">
              <div className="p-5 rounded-3xl bg-[#141414] border border-white/5 shadow-2xl space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-neutral-300 flex items-center justify-between">
                      <span>Chapter Author / Narrator</span>
                      {isSeries && (
                        <span className="text-[10px] text-orange-400/90 font-normal">
                          (This Chapter)
                        </span>
                      )}
                    </label>
                    <input
                      ...
                      placeholder="e.g., Homer, Elena Vance, Uncle Jimmy..."
                      className="w-full px-3.5 py-2 rounded-xl bg-[#1a1a1a] border border-white/10 text-xs text-neutral-200 focus:outline-hidden focus:ring-1 focus:ring-orange-400"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      placeholder="e.g., Mythology, Adventure..."
                      className="w-full px-3.5 py-2 rounded-xl bg-[#1a1a1a] border border-white/10 text-xs text-neutral-200 focus:outline-hidden focus:ring-1 focus:ring-orange-400"
                    />
                  </div>
                </div>

                {/* Genre Quick Suggestion Chips */}
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-lg text-[11px] font-medium border transition-all cursor-pointer ${
                          genre.toLowerCase() === g.toLowerCase()
                            ...
                            : "bg-[#161616] text-neutral-400 border-white/5 hover:text-white hover:bg-white/5"
                        }`}
                      >
                        {g}
                      </button>
                ...
                {/* Reading Level & Estimated Time Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {/* Reading Level */}
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-neutral-300">
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {[
                        ...
                        <button
                          ...
                          className={`px-2 py-2 rounded-xl text-[11px] font-bold border transition-all cursor-pointer text-center ${
                            readingLevel === lvl
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-neutral-300 flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => setEstimatedMinutes(Math.max(2, Math.round(slides.length * 0.7)))}
                        className="text-[10px] text-orange-400 hover:underline cursor-pointer"
                      >
                        Auto (~{Math.max(2, Math.round(slides.length * 0.7))}m)
                      </button>
                    </label>
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-full px-3.5 py-2 rounded-xl bg-[#1a1a1a] border border-white/10 text-xs text-neutral-100 focus:outline-hidden focus:ring-1 focus:ring-orange-400 font-mono"
                      />
                ...
                {/* Story Summary / Description (Shown on Library Cards) */}
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-neutral-300 flex items-center justify-between">
                    <span>Story Summary / Description (Library Overview)</span>
                  ...
                  <textarea
                    ...
                    placeholder="Enter a brief, engaging summary describing the story theme and journey for library catalog cards..."
                    className="w-full p-3 rounded-xl bg-[#1a1a1a] border border-white/10 text-xs text-neutral-200 focus:outline-hidden focus:ring-1 focus:ring-orange-400 leading-relaxed"
                  />
                </div>
                ...
                <div className="p-4 rounded-2xl bg-[#1a1a1a] border border-white/10 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-xs font-bold text-white block">
                        Search Tags (Hidden Discovery Metadata)
                      </span>
                      <span className="text-[11px] text-neutral-400 block">
                        Tags are hidden on story cards, but will match when students search in the library. Each tag is stored as <code className="text-orange-400 font-mono">[tag]</code>.
                      </span>
                    </div>
                    <span className="text-[10px] font-mono text-orange-400 font-bold bg-orange-500/10 px-2 py-0.5 rounded-full border border-orange-500/20">
                  ...
                  {/* Active Tag Chips */}
                  {tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 p-2 rounded-xl bg-[#141414] border border-white/5 max-h-28 overflow-y-auto">
                      {tags.map((t, idx) => (
                        <span
                          key={idx}
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      placeholder="Type tag(s) and press Enter, e.g. [mythology] [greek] [hero]..."
                      className="flex-1 px-3.5 py-2 rounded-xl bg-[#141414] border border-white/10 text-xs font-mono text-white focus:outline-hidden focus:ring-1 focus:ring-orange-400"
                    />
                    <button
                      type="button"
                      onClick={() => handleAddTag()}
                      className="px-3.5 py-2 rounded-xl bg-orange-500/20 hover:bg-orange-500/30 text-orange-300 text-xs font-semibold border border-orange-500/30 transition-all cursor-pointer whitespace-nowrap"
                    >
                      Add Tag
                    </button>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {[
                        ...
                        <button
                          ...
                          className="px-2 py-0.5 rounded-lg text-[10px] font-mono text-neutral-400 bg-white/5 hover:bg-white/10 hover:text-white border border-white/5 transition-colors cursor-pointer"
                        >
                          +[{sugg}]
                        </button>
                      ))}
                  ...
                  {isSeries && (
                    <div className="pt-3 border-t border-white/5 space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!seriesFolder || seriesFolder === "series") {
                                setSeriesFolder(slugify(e.target.value));
                              }
                            }}
                        ...
                        <div className="space-y-1.5">
                          <label className="text-xs font-semibold text-neutral-300 flex items-center justify-between">
                            <span>Series Author / Creator</span>
                          ...
                          <input
                            ...
                            placeholder="e.g., Homer, Various Authors, Aesop..."
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            max={100}
                            value={seriesOrder}
                            onChange={(e) => setSeriesOrder(Math.max(1, parseInt(e.target.value, 10) || 1))}
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            placeholder="e.g., Chapter I, Part II..."
                            className="w-full px-3.5 py-2 rounded-xl bg-[#141414] border border-white/10 text-xs text-neutral-100 focus:outline-hidden focus:ring-1 focus:ring-orange-400 font-mono"
                          />
                        </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!chapterFolderName || chapterFolderName === "chapter") {
                                setChapterFolderName(slugify(e.target.value));
                              }
                            }}
                            placeholder="e.g., The Fall of Troy, The Lotus Eaters..."
                            className="w-full px-3.5 py-2 rounded-xl bg-[#141414] border border-white/10 text-xs text-neutral-100 focus:outline-hidden focus:ring-1 focus:ring-orange-400"
                          />
                        </div>
                      </div>
                ...
                {/* GitHub Asset Folder Path & Auto-Populate */}
                <div className="p-4 rounded-2xl bg-[#1a1a1a] border border-white/10 space-y-3.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <FileCode className="w-4 h-4 text-orange-400" />
                      <div>
                        <span className="text-xs font-bold text-white block">
                          GitHub Asset Folder Source
                        </span>
                        <span className="text-[11px] text-neutral-400 block">
                          Pulls text, images (1..N.webp), and audio (1..N.mp3) from your GitHub repository
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {!isSeries && (
                    <div className="space-y-1.5">
                      <label className="text-xs font-semibold text-neutral-300">
                        Story Folder Name (under /public/stories/)
                      </label>
                      <input
                        type="text"
                        value={storyFolderName}
                        onChange={(e) => setStoryFolderName(slugify(e.target.value))}
                        placeholder="e.g., orpheus, clockwork_dragonfly"
                        className="w-full px-3.5 py-2 rounded-xl bg-[#141414] border border-white/10 text-xs text-neutral-100 focus:outline-hidden focus:ring-1 focus:ring-orange-400 font-mono"
                      />
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        ...
                        <input
                          type="text"
                          value={seriesFolder}
                          onChange={(e) => setSeriesFolder(slugify(e.target.value))}
                          placeholder="e.g., odyssey"
                          className="w-full px-3.5 py-2 rounded-xl bg-[#141414] border border-white/10 text-xs text-neutral-100 font-mono focus:outline-hidden focus:ring-1 focus:ring-orange-400"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-xs font-semibold text-neutral-300">
                        ...
                        <input
                          type="text"
                          value={chapterFolderName}
                          onChange={(e) => setChapterFolderName(slugify(e.target.value))}
                          placeholder="e.g., troy, lotus_eaters"
                          className="w-full px-3.5 py-2 rounded-xl bg-[#141414] border border-white/10 text-xs text-neutral-100 focus:outline-hidden focus:ring-1 focus:ring-orange-400 font-mono"
                        />
                      </div>
                    </div>
                  ...
                  {/* GitHub URL Preview */}
                  <div className="p-2.5 rounded-xl bg-[#111] border border-white/5 space-y-1">
                    <span className="text-[10px] text-neutral-400 uppercase tracking-wider font-bold block">
                      Target GitHub Repository Path:
                    </span>
                    <div className="text-[11px] text-emerald-300 font-mono break-all leading-tight">
                      {basePath}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    <button
                      type="button"
                      disabled={isFetchingFromGithub}
                      onClick={handleFetchFromGithubFolder}
                      className="px-4 py-2 rounded-xl bg-orange-400 hover:bg-orange-300 active:scale-95 text-neutral-950 font-bold text-xs transition-all shadow-md flex items-center gap-2 disabled:opacity-50"
                    >
                      {isFetchingFromGithub ? (
                        <>
                        ...
                        <>
                          ...
                          <span>Fetch & Populate from GitHub Folder</span>
                        </>
                      )}
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={handleApplyFolderPathsToAllSlides}
                      className="px-3.5 py-2 rounded-xl bg-white/5 hover:bg-white/10 active:scale-95 border border-white/10 text-neutral-300 font-semibold text-xs transition-all flex items-center gap-1.5"
                    >
                      <Check className="w-3.5 h-3.5 text-emerald-400" />
                      <span>Sync Slide URLs with Folder</span>
                    </button>
                  </div>
                  ...
                  {fetchStatusMessage && (
                    ...
                    >
                      ...
                      <span className="leading-relaxed">{fetchStatusMessage.text}</span>
                    </div>
                  )}
                </div>
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="p-5 rounded-3xl bg-[#141414] border border-white/5 shadow-2xl space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <button
                    onClick={applyFolderPathToCover}
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-400/15 hover:bg-emerald-400/25 border border-emerald-400/30 text-emerald-300 text-xs font-bold transition-all cursor-pointer"
                  >
                    <FileCode className="w-3.5 h-3.5" />
                    <span className="font-mono">Use {basePath}/images/title.webp</span>
                  </button>
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 rounded-2xl bg-[#141414] border border-white/5">
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
                ...
                {/* Sync All Slide Assets button */}
                <button
                  onClick={handleApplyFolderPathsToAllSlides}
                  className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-950/40 hover:bg-emerald-900/60 text-emerald-300 border border-emerald-500/30 text-xs font-bold transition-all cursor-pointer ml-auto"
                  title="Update all slide images to 1..N.webp and audio to 1..N.mp3 from the configured GitHub folder"
                >
                  <FileCode className="w-3.5 h-3.5" />
                  <span>Sync All with GitHub Folder</span>
                </button>
              </div>
            </div>
              ...
              <div className="lg:col-span-7 space-y-4">
                ...
                <div className="p-5 rounded-3xl bg-[#141414] border border-white/5 shadow-2xl space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    ...
                    <button
                      onClick={applyFolderPathToSlideAudio}
                      className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-400/15 hover:bg-emerald-400/25 border border-emerald-400/30 text-emerald-300 text-xs font-bold transition-all cursor-pointer"
                    >
                      <FileCode className="w-3.5 h-3.5" />
                      <span className="font-mono">
                        Use {basePath}/audio/{activeSlideIndex + 1}.mp3
                      </span>
                    </button>
                ...
                <div className="p-5 rounded-3xl bg-[#141414] border border-white/5 shadow-2xl space-y-4">
                  ...
                  <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-[#121212] border border-white/10 shadow-2xl group flex items-center justify-center">
                    <img
                      ...
                      alt={`Slide ${activeSlideIndex + 1} Artwork`}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                      referrerPolicy="no-referrer"
                    />
                    {/* Vinyl Sheen Overlay */}
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      onClick={applyFolderPathToSlideImage}
                      className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-400/15 hover:bg-emerald-400/25 border border-emerald-400/30 text-emerald-300 text-xs font-bold transition-all cursor-pointer"
                    >
                      <FileCode className="w-3.5 h-3.5" />
                      <span className="font-mono">
                        Use {basePath}/images/{activeSlideIndex + 1}.webp
                      </span>
                    </button>
