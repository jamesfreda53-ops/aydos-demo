...
import { Maximize2, Sparkles, BookOpen, Volume2 } from "lucide-react";
...
import { tokenizeStoryText } from "../utils/dictionaryTxt";
...
export const SlideViewer: React.FC<SlideViewerProps> = ({
  ...
  }, [slide.id, slide.illustrationUrl]);

  // Split paragraph into tokens (words + punctuation/whitespace + compound phrases)
  const parsedWords = React.useMemo(() => {
    return tokenizeStoryText(slide.paragraph);
  }, [slide.paragraph]);
  ...
  return (
    <div className="w-full flex flex-col flex-1 pb-4">
      ...
      >
        ...
        <img
          src={resolveStoryAssetUrl(slide.illustrationUrl)}
          alt={`Slide ${slide.slideNumber} Illustration`}
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-36 h-36 rounded-full bg-white/6 blur-3xl" />
        </div>

        {/* Top-right Frosted Glass Zoom Action button */}
        <button
          id="expand-image-btn"
          onClick={() => onImageExpand(slide.illustrationUrl, "")}
      ...
      >
        <p className="text-neutral-100 tracking-normal text-left leading-relaxed whitespace-pre-line">
