...
export const DeveloperDictionaryModal: React.FC<DeveloperDictionaryModalProps> = ({
  ...
  // Split into existing vs. new words
  ...
  const handleBatchGenerate = async () => {
    if (newWords.length === 0) {
      setStatusMessage({
        type: "info",
        text: "All story words already exist in the shared local dictionary! No duplicates needed.",
      });
      return;
    }
    ...
    try {
      ...
      const result = await batchAddWordsToDictionary(newWords, combinedContext);
      ...
      if (result.success) {
        setStatusMessage({
          type: "success",
          text: `Successfully added ${result.addedCount} new words with student definitions, parts of speech, and example sentences to dictionary.txt! (Total: ${result.totalCount} words)`,
        });
