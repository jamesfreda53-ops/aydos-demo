...
export const LibraryView: React.FC<LibraryViewProps> = ({
  ...
  const libraryItems: LibraryFeedItem[] = useMemo(() => {
    ...
    seriesMap.forEach((chapterList, sKey) => {
      ...
      // Determine series author independently from individual chapter authors:
      // 1. Explicit seriesAuthor if specified on any chapter or series metadata
      // 2. If multiple chapters have different authors, label as "Various Authors"
      // 3. Fallback to common author or first chapter's author
      const explicitSeriesAuthor = chapterList.find((c) => c.seriesAuthor?.trim())?.seriesAuthor?.trim();
      const uniqueChapterAuthors = Array.from(
        new Set(chapterList.map((c) => c.author?.trim()).filter(Boolean))
      );
      const computedSeriesAuthor =
        explicitSeriesAuthor ||
        (uniqueChapterAuthors.length > 1 ? "Various" : uniqueChapterAuthors[0] || firstChapter.author || "Unknown Author");
  ...
  const activeSeries = useMemo(() => {
    ...
    const normalized = activeSeriesId.trim().toLowerCase();
    return libraryItems.find(
      (item): item is SeriesGroupItem =>
        item.type === "series" &&
        (item.seriesId.toLowerCase() === normalized ||
          item.seriesTitle.toLowerCase() === normalized ||
          item.id.toLowerCase() === `series-${normalized}` ||
          Boolean(item.seriesFolder && item.seriesFolder.toLowerCase() === normalized))
    );
      ...
      >
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-2.5 text-left cursor-pointer transition-opacity hover:opacity-90 active:scale-95"
            title={onShowStartup ? "View Startup Screen" : undefined}
          >
            <div
            ...
            <div>
              <h1
                className="text-[16px] font-normal tracking-wide font-viaoda leading-none mb-0.5"
                style={{
                  color: THEME_COLORS.textPrimary,
