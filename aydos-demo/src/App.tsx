...
import { getScrollPosition, restoreScrollPosition } from "./utils/scrollHelper";
...
export default function App() {
  // Startup Screen state (displays on initial launch)
    ...
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed)) {
          return parsed
            .filter((s: Story) => s && s.id && typeof s.id === "string")
            .map((s: Story) => ({
              ...s,
              slides: (s.slides || []).map((slide) => {
                const { keyWords, ...rest } = slide as any;
                return rest;
              }),
            }));
        }
  ...
  // Scroll Position Tracking for Main Catalog and Series Views
  const catalogScrollPosRef = React.useRef<number>(0);
  const seriesScrollPosRef = React.useRef<Record<string, number>>({});
  ...
  // Track scroll position in real-time while browsing the library or series view
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (activeSeriesId) {
        seriesScrollPosRef.current[activeSeriesId] = y;
      } else {
        catalogScrollPosRef.current = y;
      }
    };
    ...
    if (phoneContainer) {
      phoneContainer.addEventListener("scroll", handleScroll, { passive: true });
    }

    return () => {
      window.removeEventListener("scroll", handleScroll);
      if (phoneContainer) {
        phoneContainer.removeEventListener("scroll", handleScroll);
      }
    };
  }, [currentView, activeSeriesId]);
    ...
    async function loadCatalog() {
      try {
        ...
        if (fileResult && fileResult.stories && fileResult.stories.length > 0) {
          // stories.txt is the authoritative catalog
          const builtInMap = new Map(STORIES.map((s) => [s.id, s]));
          baseList = fileResult.stories.map((manifestStory) => {
            const builtIn = builtInMap.get(manifestStory.id);
            return builtIn ? { ...builtIn, ...manifestStory } : manifestStory;
          });
        }

        // Load catalog immediately into LibraryView (titles, covers, summaries, tags are ready)
        setBuiltInStories(baseList);
      } catch (err) {
        console.warn("Failed to load stories manifest, falling back to default stories:", err);
        if (isMounted) {
          setBuiltInStories(STORIES);
        }
      }
    }
    ...
    return result.map((story) => {
      ...
      if (override) {
        return {
          ...
          seriesAuthor: override.author || story.seriesAuthor,
  ...
  // Load persistent flashcards from /public/flashcards.json (via server API) on start
  useEffect(() => {
    let isMounted = true;
  ...
  // Handle Active Series View Toggle with Scroll Preservation
  const handleActiveSeriesChange = useCallback((newSeriesId: string | null) => {
    const currentY = getScrollPosition();
    if (activeSeriesId) {
      seriesScrollPosRef.current[activeSeriesId] = currentY;
    } else {
      catalogScrollPosRef.current = currentY;
    }

    setActiveSeriesId(newSeriesId);

    if (newSeriesId) {
      // Opening a series -> restore previously saved series position or start at top
      const targetY = seriesScrollPosRef.current[newSeriesId] || 0;
      restoreScrollPosition(targetY, "auto");
    } else {
      // Returning to main catalog from series view -> restore main catalog scroll position
      const targetY = catalogScrollPosRef.current || 0;
      restoreScrollPosition(targetY, "auto");
    }
  }, [activeSeriesId]);
  ...
  const handleSelectStory = (story: Story) => {
    ...
    // Capture current scroll position before leaving Library view
    const currentY = getScrollPosition();
    if (activeSeriesId) {
      seriesScrollPosRef.current[activeSeriesId] = currentY;
    } else {
    ...
    // 1. Immediately display the TitleSlideViewer with the selected story's metadata
    setActiveStory(story);
    setCurrentSlideIndex(0); // Starts on Title Slide (Slide 0)
    setCurrentView("reader");
    restoreScrollPosition(0, "instant");

    // 2. Hydrate full story text & slide assets in background without blocking the UI
    const targetStoryId = story.id;
    hydrateStoryWithTextFile(story).then((hydrated) => {
      setActiveStory((current) => {
        if (current && current.id === targetStoryId) {
          return hydrated;
        }
        return current;
      });
      // Also update in allStories cache so reopening it later is instant
      setBuiltInStories((prev) =>
        prev.map((s) => (s.id === targetStoryId ? hydrated : s))
      );
    }).catch((err) => {
      console.warn("Background story hydration failed:", err);
    });
  };

  // Exits a story or quiz back to the Series screen (if chapter) or Main Library catalog (if standalone)
  const handleExitStoryToLibrary = (storyToExit?: Story | null) => {
    audioEngine.stop();
    setIsQuizOpen(false);

    const story = storyToExit || activeStory;
    const isSeries = Boolean(
      story?.seriesTitle?.trim() ||
      story?.seriesId?.trim() ||
      story?.seriesFolder?.trim() ||
      story?.chapterFolder?.trim() ||
      activeSeriesId
    );

    const targetSeriesId = story?.seriesId?.trim() || story?.seriesTitle?.trim() || activeSeriesId;

    if (isSeries && targetSeriesId) {
      setActiveSeriesId(targetSeriesId);
      setActiveStory(null);
      setCurrentSlideIndex(0);
      setCurrentView("library");

      const targetY = seriesScrollPosRef.current[targetSeriesId] || 0;
      restoreScrollPosition(targetY, "auto");
    } else {
      setActiveSeriesId(null);
      setActiveStory(null);
  ...
  const handleBackToLibrary = () => {
    audioEngine.stop();
    if (isQuizOpen) {
      handleExitStoryToLibrary();
      return;
    }
    // If on a content slide in reader, go back to Chapter Title screen (slide 0)
    if (currentSlideIndex > 0) {
      setCurrentSlideIndex(0);
      restoreScrollPosition(0, "smooth");
      return;
    }
    // If on Chapter Title screen (slide 0), go back to Series container screen (or library catalog)
    handleExitStoryToLibrary();
  };
  ...
  const handleSaveCustomStory = (story: Story) => {
    ...
    if (updatedStory.seriesId || updatedStory.seriesTitle) {
      ...
      const seriesAuthor =
        updatedStory.seriesAuthor !== undefined && updatedStory.seriesAuthor.trim() !== ""
          ? updatedStory.seriesAuthor
          : existingSeriesOverride?.author || "";

      const seriesData: SeriesData = {
        seriesId: updatedStory.seriesId || sKey,
        seriesTitle: updatedStory.seriesTitle || "",
        seriesDescription: updatedStory.seriesDescription || existingSeriesOverride?.seriesDescription || "",
        seriesFolder: updatedStory.seriesFolder || existingSeriesOverride?.seriesFolder || "",
        seriesCoverImage: updatedStory.seriesCoverImage || existingSeriesOverride?.seriesCoverImage || "",
        author: seriesAuthor,
        genre: updatedStory.genre || existingSeriesOverride?.genre || "Adventure",
        readingLevel: (updatedStory.readingLevel || existingSeriesOverride?.readingLevel || "Intermediate") as any,
      };
    ...
    setCurrentView("library");
    const targetY = activeSeriesId
      ? (seriesScrollPosRef.current[activeSeriesId] || 0)
      : (catalogScrollPosRef.current || 0);
    restoreScrollPosition(targetY, "auto");
  };
      ...
      allStories.forEach((st) => {
        ...
        if (matchSeries) {
          const updatedStory: Story = {
            ...
            seriesAuthor: seriesData.author || st.seriesAuthor,
            author: st.author,
  ...
  const handleSaveWord = (wordDef: WordDefinition) => {
    if (!savedWords.some((w) => w.word.toLowerCase() === wordDef.word.toLowerCase())) {
      ...
      // Directly persist new entry to /public/flashcards.json in AI Studio
      addFlashcardToServer(newSaved);
    }
  };
  ...
  const handleRemoveSavedWord = (word: string) => {
    setSavedWords((prev) => {
      ...
      deleteFlashcardFromServer(word);
      return next;
    });
  };

  const handleImportWords = (imported: SavedWord[]) => {
    setSavedWords((prev) => {
      const map = new Map<string, SavedWord>();
      prev.forEach((w) => map.set(w.word.toLowerCase(), w));
      imported.forEach((w) => map.set(w.word.toLowerCase(), w));
      const merged = Array.from(map.values());
      saveAllFlashcards(merged);
      return merged;
    });
  };
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showStartup && (
          <StartupScreen onStart={() => setShowStartup(false)} />
        )}
      </AnimatePresence>
      <MobileFrame
      ...
      {currentView === "library" || !activeStory ? (
        <LibraryView
          ...
          onShowStartup={() => setShowStartup(true)}
        />
      ) : (
      ...
      {/* Tap-to-define Dictionary Modal (rendered in front of Reader and Study window) */}
      <DictionaryModal
      ...
      {activeStory && (
        <ComprehensionQuizModal
          isOpen={isQuizOpen}
          onClose={() => handleExitStoryToLibrary(activeStory)}
          story={activeStory}
          onReread={() => {
            audioEngine.stop();
            setIsQuizOpen(false);
          ...
          onBackToLibrary={() => handleExitStoryToLibrary(activeStory)}
        />
      )}
