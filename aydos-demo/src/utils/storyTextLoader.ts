...
import { fetchQuizForStory } from "./quizStorage";
...
export async function hydrateStoryWithTextFile(story: Story): Promise<Story> {
  // Genuinely local-only custom creations have no real GitHub folder to fetch
  // from, so there's nothing to hydrate. Anything with a real manifest entry
  // SHOULD still re-hydrate even after a local edit -- GitHub is now the
  // durable source of truth, and the per-field merging below already protects
  // locally-edited text from being clobbered by stale defaults.
  if (story.id.startsWith("custom-")) {
    return story;
  }
  ...
  try {
    ...
    if (!rawText) {
      ...
      const quizResult = await fetchQuizForStory({
        seriesFolder: hasSeriesAndChapter ? story.seriesFolder : undefined,
        chapterFolder: hasSeriesAndChapter ? story.chapterFolder : undefined,
        storyFolder: !hasSeriesAndChapter ? sFolder : undefined,
      });
      ...
      return {
        ...
        titleSlide: story.titleSlide
          ...
          : undefined,
        quizQuestions: quizResult.questions.length > 0 ? quizResult.questions : story.quizQuestions,
      };
    }
    ...
    // Fetch real quiz questions if a quiz.json exists for this story/chapter,
    // replacing the generic auto-generated placeholder question
    const quizResult = await fetchQuizForStory({
      seriesFolder: hasSeriesAndChapter ? story.seriesFolder : undefined,
      chapterFolder: hasSeriesAndChapter ? story.chapterFolder : undefined,
      storyFolder: !hasSeriesAndChapter ? sFolder : undefined,
    ...
    if (quizResult.questions.length > 0) {
      updatedStory.quizQuestions = quizResult.questions;
    }
